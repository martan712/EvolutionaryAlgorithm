...
class Population:
    ...
    def local_search(self, max_depth: float):
        for route in self.routes:
            route.best_2_opt_local_search(max_its=max_depth)
    ...
    def plot_route(self, title="Lecture Route", filename=""):
...
def run_experiment(params):
    ...
    filename = f"results/tsp_npop={n_population}_K={K}_mu={mu}_depth={max_depth}.png"
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for future in futures:
            try:
                ...
                print(f"Completed task with result: {results[-1]['best_distance']:.2f}")
...
def run_experiment_for_depths(cities, n_population, K, mu, depths, timeout):
    results = []
    ...
    for max_depth in depths:
        ...
        while perf_counter() - t0 < timeout:
            ...
            # Record current best distance
            best_route = max(population.routes, key=lambda r: r.fitness())
            current_distance = best_route.distance()
            current_time = perf_counter() - t0
            time_series.append((current_time, current_distance))
        
        results.append({
            'max_depth': max_depth,
            'time_series': time_series,
            'best_distance': time_series[-1][1]  # Final distance
        })
...
def plot_combined_distance_vs_time(results, n_population, K, mu):
    ...
    for result in results:
        ...
        plt.plot(
            times, 
            distances, 
            label=f"Depth={result['max_depth']} (Best: {result['best_distance']:.2f})",
            linewidth=1.5
        )
    ...
    plt.title(
        f"TSP Optimization Progress\n"
        f"n_pop={n_population}, K={K}, μ={mu}"
    )
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/combined_depth_comparison.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    # Save raw data
    import json
    with open('results/depth_comparison_results.json', 'w') as f:
        json.dump(results, f, indent=2)
